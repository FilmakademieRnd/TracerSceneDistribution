...
void AVPETModule::HasChangedIsCalled(AbstractParameter* param)
{
	...
	UE_LOG(LogTemp, Error, TEXT("change is called with param dataSyze %d"), param->dataSize());
	
	bool paramInList = VPET_ModifiedParameterList.Contains(param);
	
	if (paramInList)
	{
			VPET_ModifiedParameterList.Remove(param);
			VPET_modifiedParametersDataSize -= param->dataSize();
	}
	
	else if (!paramInList)
	{
		VPET_ModifiedParameterList.Add(param);
		VPET_modifiedParametersDataSize += param->dataSize();
	}
	
	UE_LOG(LogTemp, Error, TEXT("m_modifiedParametersDataSize = %d"), VPET_modifiedParametersDataSize);
}

void AVPETModule::CreateParameterMessage()
{
	...
	//start of the msg after the header (pos 3)
	int start = 3;
	for (auto &parameter: VPET_ModifiedParameterList )
	{
		...
		//parameter ID(location, rotation, scale...)
		shortVal = parameter->_id;
		memcpy(responseMessageContent, &shortVal, sizeof(uint16_t)); responseMessageContent += sizeof(uint16_t);

		// Parameter VPET type!
		intVal = static_cast<uint8_t>(parameter->_type);
		memcpy(responseMessageContent, &intVal, sizeof(uint8_t)); responseMessageContent += sizeof(uint8_t);

		// the length of 1 parameter msg without the header.
		uint32_t intVal2 = length;
		memcpy(responseMessageContent, &intVal2, sizeof(uint32_t)); responseMessageContent += sizeof(uint32_t);

		// the serialization of the parameter DATA!

		parameterData = (char*)malloc(parameter->dataSize());
		...
		{
			float floatValue;
			std::memcpy(&floatValue, startOfData + i * sizeof(float), sizeof(float));
			UE_LOG(LogTemp, Warning, TEXT("Float value from VPET<ODULE.CPP %d: %f"), i, floatValue);
		}
		free(parameterData);
...
void AVPETModule::Tick(float DeltaTime)
{
	...
	if (VPET_ModifiedParameterList.Num()>0)
	{
		...
		VPET_ModifiedParameterList.Empty();
		VPET_modifiedParametersDataSize = 0;
	}
...
void AVPETModule::ParseParameterUpdate(std::vector<uint8_t> kMsg)
{
	...
	// Grab a byte
	/*uint8_t fByte;
	for (size_t j = 0; j < kMsg.size(); j++)
	{
		fByte = kMsg[j];
		DOL(LogBasic, Error, "[Parse] Byte: %d, value: %d", j, fByte);
	}*/
	...
	for (int i = 3; i < kMsg.size() ; )
	{
		...
		//ParameterType paramType = (ParameterType)kMsg[i+4];
		//DOL(LogBasic, Log, "[SYNC Parse] Param type: %d", paramType);

		// grab scene object and its parameters
		USceneObject* sceneObj = VPET_SceneObjectList[objectID];
		...
		if (paramID < tempArray->Num())
		{
			...
			std::vector<uint8_t> subMsg = std::vector(kMsg.begin()+i+10, kMsg.begin()+i+length);
...
void AVPETModule::HandleOnActorSelectionChanged(const TArray<UObject*>& NewSelection, bool bForceRefresh)
{
	...
	for (UObject* obj : NewSelection)
	{
		...
		// Send lock
		if (objI != INDEX_NONE && !VPET_SceneObjectList[objI]->_lock)
...
bool AVPETModule::buildLocation(AActor* prim)
{
	...
	if (UseEditableTag)
	{
		...
		if (prim->ActorHasTag(TEXT("editable")) || tweakRot)
		{
			...
			obj->SetID(VPET_SceneObjectList.Num());
			obj->SetcID(m_id);
			VPET_SceneObjectList.Add(obj);
...
void AVPETModule::buildNode(NodeGeo* node, AActor* prim)
{
	...
	DOL(LogGeoBuild, Warning, "[DIST buildNode] Num LOD resources: %d", staticMesh->UStaticMesh::GetRenderData()->LODResources.Num());
	...
	if (node->geoId < 0)
	{
		...
		if (!staticMesh->UStaticMesh::GetRenderData()) return;


		if (staticMesh->UStaticMesh::GetRenderData()->LODResources.Num() > 0)
		{
			...
			FStaticMeshLODResources& resource = staticMesh->UStaticMesh::GetRenderData()->LODResources[staticMesh->UStaticMesh::GetRenderData()->LODResources.Num() - 1];
			...
			for (size_t j = 0; j < smVP.GetNumVertices(); j++) {
				...
				FVector2f vUV = smVP.GetVertexUV(j, 0);
	...
	if (true)
	{
		...
		if (cbMat)
		{
			...
			FColorMaterialInput cbColor = cbMat->GetEditorOnlyData()->BaseColor;
			if (cbColor.IsConnected())
			{
				...
				// Casting experiments
				UMaterialExpressionParameter* exParam = Cast<UMaterialExpressionParameter>(cbExpr);
				UMaterialExpressionConstant3Vector* exConst3 = Cast<UMaterialExpressionConstant3Vector>(cbExpr);
				UMaterialExpressionConstant4Vector* exConst4 = Cast<UMaterialExpressionConstant4Vector>(cbExpr);
				UMaterialExpressionTextureBase* exTextu = Cast<UMaterialExpressionTextureBase>(cbExpr);
...
void AVPETModule::DecodeLockMessage(int16_t* objID, bool* lockState)
{
	...
	USceneObject* sceneObj = VPET_SceneObjectList[*objID];
