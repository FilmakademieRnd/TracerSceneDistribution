...
#define OSD(debugColor, debugString, ...) \
if (GEngine && VerboseDisplay) { \
GEngine->AddOnScreenDebugMessage(-1, 5.0f, debugColor, FString::Printf(TEXT(debugString), ##__VA_ARGS__)); \
}
...
public:
	...
	void UpdateTime();
	void queueSyncMessage(uint8_t time);
